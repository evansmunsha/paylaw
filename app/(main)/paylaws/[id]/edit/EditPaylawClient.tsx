...
export default function EditPaylawClient({ paylaw, employees }: Props) {
  ...
  const [month, setMonth]             = useState(paylaw.month)
  const [year, setYear]               = useState(paylaw.year)
  const [preparedBy, setPreparedBy]   = useState(paylaw.preparedBy)
  const [foodExpense, setFoodExpense] = useState(String(paylaw.foodExpense))
  const [otherDeduct, setOtherDeduct] = useState(String(paylaw.otherDeduct))

  // Pre-fill worker rows with existing attendance data
  ...
  async function handleSave(status: 'draft' | 'done') {
    ...
    try {
      const res = await fetch(`/api/paylaws/${paylaw.id}`, {
        method: 'PATCH',
  ...
  return (
    <div className="p-6 flex flex-col gap-5">
      ...
      <div className="flex items-center gap-3 bg-blue-50 border border-blue-100
        ...
        Your attendance marks are saved exactly as you left them.
        Keep marking and click <strong className="mx-1">Save draft</strong>
        to come back later, or <strong className="mx-1">Save &amp; done</strong>
        when the month is finished.
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
          ...
          <div className="flex items-center gap-4 text-xs text-gray-500">
            ...
            <span className="text-gray-400">· Click any day to toggle</span>
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5
        ...
        <div className="flex gap-8">
          ...
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-wide mb-1">Total days</p>
            <p className="text-xl font-semibold text-amber-700">{totalDays}</p>
          </div>
          <div>
