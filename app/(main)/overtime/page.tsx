  ...
  const [overtimes, settings] = await Promise.all([
    plan !== 'free'
      ? prisma.overtime.findMany({
          where: session.user.role === 'foreman'
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="p-4 md:p-6 flex flex-col gap-5">
        {plan === 'free' ? (
          <UpgradeBanner
            title="Overtime tracking is not available on the free plan"
            message="Upgrade to Starter to track overtime hours, calculate overtime pay and download overtime PDFs for your workers."
            feature="Unlimited overtime sheets, per-worker hours per day, overtime PDF download"
          />
        ) : (
